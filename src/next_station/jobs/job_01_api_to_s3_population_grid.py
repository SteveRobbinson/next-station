...
def ingest_population_grid_to_s3():
    ...
    try:
        ...
        s3_object_metadata = s3.get_object_metadata(file_path=settings.aws.s3_population_grid_file_name)
        ...
        if s3_object_metadata != api_object_metadata:
            ...
            population_grid = runner(api_url=population_grid_file_url, method='get', stream=True)

            s3.upload_data_to_s3(file_name=settings.aws.s3_population_grid_file_name,
