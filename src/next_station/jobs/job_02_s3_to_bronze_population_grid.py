...
def load_population_grid_to_databricks():
    ...
    try:
        ...
        spark.save_df_in_databricks(df_exploded, settings.databricks.population_grid_bronze_fqn)

        logger.info("Successfully completed population grid load to Databricks")    

    except Exception:
        logger.error("An error occurred while processing and uploading data to databricks")
        raise
