...
def load_railway_stations_to_databricks() -> None:
    ...
    try:
        ...
        df_raw = spark.read_from_s3(
            aws_s3_path=settings.aws.railway_stations_uri, data_format="json"
        )

        logger.info("Table transformations")
        df_processed = explode_and_flatten(
            df=df_raw, explode_by=settings.aws.railway_file_explode_by
        )

        logger.info("Uploading table to databricks")
        spark.save_df_in_databricks(
            df=df_processed, table_name=settings.databricks.railway_stations_bronze_fqn
        )
